import type { SeasonTeams } from '../types';
import type { ResolvedRanking } from '../utils/rankings';
import { RankingBadge } from './RankingBadge';

interface TeamNameProps {
  team: string;
  teams: SeasonTeams | null | undefined;
  rankings?: Record<string, ResolvedRanking> | null;
  className?: string;
}

/**
 * Displays a team's full name by default, switching to its CFBD abbreviation
 * (e.g. "TTU") on narrow viewports where space is constrained. Optionally
 * shows a muted ranking badge (AP/CFP) next to the name.
 */
export function TeamName({ team, teams, rankings, className = '' }: TeamNameProps) {
  const meta = teams?.teams[team];
  const full = meta?.display_name ?? team;
  const abbreviation = meta?.abbreviation ?? team;

  return (
    <span className={className}>
      <span className="hidden sm:inline">{full}</span>
      <span className="sm:hidden">{abbreviation}</span>
      {rankings && <RankingBadge team={team} rankings={rankings} className="ml-1 align-super" />}
    </span>
  );
}
